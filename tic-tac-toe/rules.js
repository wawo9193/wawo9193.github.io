...
function begin_play(){
	...
	if (isEmpty(name1.value) || isEmpty(name2.value)) {
		...
		name1.value += ' (X)';
		name2.value += ' (O)';
		turn_info.innerHTML = "Turn for: " + helper();
	} else {
...
function reset_play(){
	...
	for (var i=0; i<9; i++) {
		if (board_state[i] > -1) {
			...
			if (i < 3) {
				//a
				document.getElementById('A' + (i+1).toString()).innerHTML = 'A' + (i+1).toString();
			} else if (i < 6) {
				//b
				document.getElementById('B' + (i-2).toString()).innerHTML = 'B' + (i-2).toString();
			} else {
				document.getElementById('C' + (i-5).toString()).innerHTML = 'C' + (i-5).toString();
			}
		}
	}
	started = false;
...
function winner() {
	...
	for (var j=0; j<3; j++) {
		if (board_state[j] != -1 &&
			board_state[j] == board_state[j+3] && 
			board_state[j+3] == board_state[j+6]) {
			w = board_state[j];
		}
	}
	if (board_state[0] != -1 &&
...
function play() {
	...
	else if (move.match('^[A-C][1-3]$') && board_state[i] < 0) {
		// alert('success!');
		document.getElementById(move).innerHTML = helper();
		board_state[i] = whose_move();
		toggle_move();
		document.getElementById('turn_info').innerHTML = "Turn for: " + helper();
		document.getElementById('move_text_id').value = '';
